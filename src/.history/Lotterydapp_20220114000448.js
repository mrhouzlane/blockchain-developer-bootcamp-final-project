...
winnerIs.onclick = async () => {
    ...
    var winneR = await lotteryContract.methods.pickWinner().call({ from: ethereum.selectedAddress });
...
window.addEventListener('load', async () => {
    ...
    if (state == "2") {
        console.log("Winner announcing")
        let closedState = document.getElementById("Loading")
        closedState.innerHTML = "Winner announcing";
    }
})


window.addEventListener('load', async () => {
    const payable(player) = await lotteryContract.methods.winner().call()
    if (payable(player) == address(0)) {
        let playerLL = document.getElementById("player")
        playerLL.innerHTML = "No winner yet"
    }
    else {
        let playerLL = document.getElementById("player")
        playerLL.innerHTML = "The winner is :" + player;
