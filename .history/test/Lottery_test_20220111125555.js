...
contract('Lottery', async (accounts) => {
    ...
    it('should let non manager accounts to enter Lottery', async () => {
        let instance = await Lottery.deployed();
        let players = instance.players()
        player1 = await instance.enter({ from: accounts[1], value: web3.utils.toWei('1', 'ether') })
        assert(players, player1.address())
    })
