...
contract('Lottery', async (accounts) => {
    ...
    it('should reward the winner 90% of the balance', async () => {
        let instance = await Lottery.deployed();
        let balance = await instance.getBalance({ from: accounts[0] });
        await instance.rewardWinner({ from: accounts[0] });
        let balanceWinner = 5000000000000000000 * 0.9;
        let winner = instance.winner()
        assert.equal((winner).balance, balanceWinner.toString())
        assert.equal('5000000000000000000', balance.toString());   // we considering that account 1 entered 3 times as seen below 
    })
